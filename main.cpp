...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&infoQueue))))
	{
		...
		//抑制するメッセージのID
		D3D12_MESSAGE_ID denylds[] = {
			//Windows11でのDXGIデバッグレイヤーとDX12デバッグレイヤーの相互作用バグによるエラーメッセージ
			//https://stackoverflows.com/question/69805245/directx-12-application-is-crashing-in-windows-11
			D3D12_MESSAGE_ID_RESOURCE_BARRIER_MISMATCHING_COMMAND_LIST_TYPE
		};
		//抑制するレベル
		D3D12_MESSAGE_SEVERITY severities[] = { D3D12_MESSAGE_SEVERITY_INFO };
		D3D12_INFO_QUEUE_FILTER filter{};
		filter.DenyList.NumIDs = _countof(denylds);
		filter.DenyList.pIDList = denylds;
	...
	//コマンドアロケータの生成がうまくいかなっかったので起動できない
	assert(SUCCEEDED(hr));
...
#pragma region 描画処理
			...
			barrier.Transition.pResource = swapChainResources[backBuffferIndex];
			...
			//描画先のRTVを設定する
			commandList->OMSetRenderTargets(1, &rtvHandles[backBuffferIndex], false, nullptr);
			//指定した色で画面全体をクリアする
			...
			//TranaitionBarrierを張る
			commandList->ResourceBarrier(1, &barrier);

			//コマンドリストの内容を確率させる。すべてのコマンドを積んでからCloseすること
